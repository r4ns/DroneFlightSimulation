package tests;

import org.junit.Test;

import drone.DoubleCubeDron;

public class DroneAdvancedTest {
	/*
	DoubleCubeDrone drone; 
	private int[]  boundaries = {50,50,50}; 
	private int[] startCoordinates = {30,0,30}; 
	
	@Test
	public void testPutanja(){
		drone = new DoubleCubeDrone(boundaries, startCoordinates);
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving up*********");
		for(int i=0;i<12;i++)
		{
			System.out.println(drone.moveUp());
		}
		
		
		
		
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving right*********");
		for(int i=0;i<21;i++)
		{
			System.out.println(drone.moveRight());
		}
		
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving up*********");
		for(int i=0;i<23;i++)
		{
			System.out.println(drone.moveUp());
		}
		
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving forth (back, jer se z smanjuje)*********");
		for(int i=0;i<16;i++)
		{
			System.out.println(drone.moveBack());
		}
		
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving left*********");
		for(int i=0;i<12;i++)
		{
			System.out.println(drone.moveLeft());
		}
		
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving forth(Back)*********");
		for(int i=0;i<5;i++)
		{
			System.out.println(drone.moveBack());
		}
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving left*********");
		for(int i=0;i<18;i++)
		{
			System.out.println(drone.moveLeft());
		}
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving up*********");
		for(int i=0;i<17;i++)
		{
			System.out.println(drone.moveUp());
		}
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving back - forth*********");
		for(int i=0;i<21;i++)
		{
			System.out.println(drone.moveForth());
		}
		
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving left*********");
		for(int i=0;i<22;i++)
		{
			System.out.println(drone.moveLeft());
		}
		
		
		System.out.println("DoubleCubeDrone position: ");
		System.out.println("*********Moving down*********");
		for(int i=0;i<20;i++)
		{
			System.out.println(drone.moveDown());
		}
	}
	
	*/
	
	
	
	
	
	
	
	
	
	

}
