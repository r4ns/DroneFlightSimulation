...
public class Cube {
	private int[] minCoordinates; // donja tacka dijagonale
	private int cubeSideLength; // duzina stranice 
	private int[] maxCoordinates; // gornja tacka dijagonale 
	

	public Cube(int[] cubeStartCoordinates, int side) {
		this.minCoordinates = cubeStartCoordinates;
		this.cubeSideLength = side;
		this.maxCoordinates = new int[] { minCoordinates[0] + cubeSideLength, minCoordinates[1] + cubeSideLength,
				minCoordinates[2] + cubeSideLength };
	...
	public boolean checkIfCubesAreTouchingFromInside(Cube cube) {
		if (checkIfCubesAreTouching(cube) && checkCoordinates(cube.getCenterOfCube()) && !checkCubeIntersection(cube))
			return true;
